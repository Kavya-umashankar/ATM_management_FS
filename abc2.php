...
$with="";
$b1=(int)$b +(int)$deposit;
  //echo shell_exec("insert.exe  \"$accno\" \"$n\"  \"$acctype\" \"$b1 \"" );
  //custmain, c1index -->main data(balance)
  $f= shell_exec("replace.exe \"$accno\" ");
  echo shell_exec("insert.exe  \"$accno\" \"$n\"  \"$a\" \"$b1 \"" );
  //cust,cindex -->manager
  echo shell_exec("insert1.exe  \"$tid1\"  \"$accno\" \"$n\" \"$entry\" \"$exit1\"  \"$with\" \"$deposit\" \"$a\" \"$b1 \" " );
  //hashfile
//echo shell_exec("hashing.exe  \"$tid1\"  \"$accno\" \"$n\" \"$entry\" \"$exit1\"  \"$with\" \"$deposit\" \"$a\" \"$b1\" " );
print_r(explode("|",$f));
?>
<html>
<body>
